...
void sprawdzSciany(short pole) {
	...
	sciany = 100*(~GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_12)) + 10* (~GPIO_ReadInputDataBit(GPIOB, GPIO_Pin_4))+ (~GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_4));
...
/*void turnLeft() {
	PRAWY_PRZOD;
	LEWY_TYL;
	int ttemp1 = obroty1, ttemp2 = obroty2;
	double K = 1, Ti = 1;

	double target = 27;

	while (1) {
		temp1 = (TIM_GetCounter(TIM3));	//lewy
		temp2 = (TIM_GetCounter(TIM1));	//prawy
		if (temp1 != previousTemp1) {
			previousTemp1 = temp1;
			obroty1 += 1;
		}
		if (temp2 != previousTemp2 && temp2 % 2 == 0) {
			previousTemp2 = temp2;
			obroty2 += 1;
		}
...
void goForward() {
	...
	while (1) {
		...
		if (temp2 != previousTemp2) {
			...
			if (temp2 < previousTemp2
					|| ((previousTemp2 == 0) && (temp2 == 5))) {
				GPIO_SetBits(GPIOC, GPIO_Pin_13);
			} else {
				GPIO_ResetBits(GPIOC, GPIO_Pin_13);
			}
			previousTemp2 = temp2;
			//obroty2>20?GPIO_SetBits(GPIOC, GPIO_Pin_15):GPIO_ResetBits(GPIOC, GPIO_Pin_15);
		}

		uchyb1 = target - obroty1;
		uchyb2 = target - obroty2;
		...
		TIM_SetCompare2(TIM4, uchyb2 > 0 ? K * uchyb2 : -K * uchyb2);
		TIM_SetCompare4(TIM4, uchyb1 > 0 ? K * uchyb1 : -K * uchyb1);
	}
...
void goToTarget() {
	...
	while (1) {
		...
		//TIM_SetCompare2(TIM4, uchyb2 > 0 ? K*(1+Td*((uchyb2-uchybPrev2)/(time-timePrevious))) * uchyb2 : -K*(1+Td*((uchyb2-uchybPrev2)/(time-timePrevious)))* uchyb2);
		//TIM_SetCompare4(TIM4, uchyb1 > 0 ? K*(1+Td*((uchyb1-uchybPrev1)/(time-timePrevious))) * uchyb1 : -K*(1+Td*((uchyb1-uchybPrev1)/(time-timePrevious))) * uchyb1);
		TIM_SetCompare2(TIM4,
				uchyb2 > 0 ?
						K * (1 + Td * ((uchyb2 - uchybPrev2) / (0.001)))
								* uchyb2 :
						-K * (1 + Td * ((uchyb2 - uchybPrev2) / (0.001)))
								* uchyb2);
		TIM_SetCompare4(TIM4,
				uchyb1 > 0 ?
						K * (1 + Td * ((uchyb1 - uchybPrev1) / (0.001)))
								* uchyb1 :
						-K * (1 + Td * ((uchyb1 - uchybPrev1) / (0.001)))
								* uchyb1);
		uchybPrev1 = uchyb1;
...
void go(char nextKierunek){
	...
	case 1: target1+=27+70; target2-=27+70;
	break;
	case 2: target1-=27+70; target2+=27+70;
	break;
	case 3: target1-=54+70; target2+=54+70;
	break;
	default: target1+=70; target2+=70;
	}
}
...
#define PEEK stackPeek(&stos)
	...
	while (pola[PEEK].odleglosc != 0) {
		/*TIM_SetCompare2(TIM4, 50);
		 ...
		 delay_ms(3000);
		 GPIO_ResetBits(GPIOB, GPIO_Pin_6); // lewy
		...
		/*if (jedz && !stop) {
		 ...
		 if(obroty/10*2*3.1415*0.021 > 0.17 0.0793){ // na turnieju zmienic na 18
