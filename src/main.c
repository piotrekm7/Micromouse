...
#define PEEK stackPeek(&stos)
  ...
  int lamp = 0;
  while (1) {
    char powrot = 0; // need it for checking if the robot has turned away and is
                     // returning , then we must pop from stack last position
    while (pola[PEEK].odleglosc != 0 || uchyb1 != 0 || uchyb2 != 0) {
      ...
      if (!przyciskFlag) {
        /*
         * Resetting all necessary variables to make the robot solve another
         * maze
         */
        controllerInit();
        stos.size = 0;
      ...
      if (temp1 != previousTemp1) {
        obroty1 += (temp1 < previousTemp1 || (temp1 == 5 && previousTemp1 == 0))
                       ? 1
                       : -1;
        previousTemp1 = temp1;
      }
      if (temp2 != previousTemp2) {
        obroty2 += (temp2 > previousTemp2 || (temp2 == 0 && previousTemp2 == 5))
                       ? 1
                       : -1;
        previousTemp2 = temp2;
      }

      /**timeStep is time beetween signals calculations , used for derivative
       * and
       * integral. It isn't done the correct way , the time beetween
       * calculations
       * will differ, there should be an interrupt which can invoke signals
       * calculation, we'll introduce it in future
       */
      int timeStep =
          50; // in miliseconds, it must be divided by 1000 in calculation
      ...
      if (time > timeStep) {
        ...
        signal1 =
            1.0 * K *
            (uchyb1 + Td * (uchyb1 - uchybPrev1) / (1.0 * timeStep / 1000) +
             Ti * uchybSum1);

        signal2 =
            1.0 * K *
            (uchyb2 + Td * (uchyb2 - uchybPrev2) / (1.0 * timeStep / 1000) +
             Ti * uchybSum2);
      ...
      if (fabs(uchyb1) < 4 && fabs(uchyb2) < 4 && przyciskFlag &&	//IDEALLY <1 CHANGE
        ...
        if (next.kierunek == previousKierunek) {
          if (powrot) {
            stackPop(&stos);
          } else {
